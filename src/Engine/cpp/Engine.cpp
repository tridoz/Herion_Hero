...
void Engine::InitWindows()
{
    ...
    main_window->SetMenu(Strings::Menus::Main_Window::Names::general_settings_menu_name,
    ...
    main_window->SetMenu(Strings::Menus::Main_Window::Names::graphics_settings_menu_name,
                         graphics_settings_menu);

    main_window->SetMenu(Strings::Menus::Main_Window::Names::audio_settings_menu_name,
                         audio_settings_menu);
...
void Engine::SetMenusDimensions()
{
    ...
    main_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                            static_cast<float>(main_window->GetHeight()));

    general_settings_menu->SetTextureManager(main_texture_manager);
    general_settings_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                                        static_cast<float>(main_window->GetHeight()));

    graphics_settings_menu->SetTextureManager(main_texture_manager);
    graphics_settings_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                                         static_cast<float>(main_window->GetHeight()));

    audio_settings_menu->SetTextureManager(main_texture_manager);
    audio_settings_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                                      static_cast<float>(main_window->GetHeight()));

    pause_menu->SetTextureManager(main_texture_manager);
    pause_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                             static_cast<float>(main_window->GetHeight()));

    editor_menu->SetTextureManager(main_texture_manager);
    editor_menu->SetDimension(static_cast<float>(main_window->GetWidth()),
                              static_cast<float>(main_window->GetHeight()));
...
void Engine::InitInputProcessor()
{
    ...
    processor->SetMenus(Strings::Menus::Main_Window::Names::general_settings_menu_name,
                        general_settings_menu);

    processor->SetMenus(Strings::Menus::Main_Window::Names::graphics_settings_menu_name,
                        graphics_settings_menu);

    processor->SetMenus(Strings::Menus::Main_Window::Names::audio_settings_menu_name,
                        audio_settings_menu);
    ...
    processor->SetMenus(Strings::Menus::Level_Editors_Window::Names::texture_selection_menu_name,
                        texture_selection_menu);

    processor->SetMenus(Strings::Menus::Level_Editors_Window::Names::action_selection_menu_name,
                        action_selection_menu);

    processor->SetMenus(Strings::Menus::Level_Editors_Window::Names::entity_selection_menu_name,
                        entity_selection_menu);

    processor->SetTextureManager(Strings::TextureManagers::Names::main_texture_manager_name,
                                 main_texture_manager);

    processor->SetTextureManager(Strings::TextureManagers::Names::texture_selection_menu_name,
                                 texture_selection_texture_manager);

    processor->SetTextureManager(Strings::TextureManagers::Names::action_selection_menu_name,
                                 action_selection_texture_manager);

    processor->SetTextureManager(Strings::TextureManagers::Names::entity_seleciton_menu_name,

...
auto JSONParser::animations::SetConfigFile(const std::string& filename) -> void {
...
auto JSONParser::animations::GetAnimationNumbers() -> int {
    if (!animation_file.contains("animations")) {
        THROW_FILE_MALFORMED(animation_file_path + "\t=> animations");
    }

    return static_cast<int>(animation_file["animations"].size());
}

auto JSONParser::animations::GetAnimationElementsFields(const int num_animation)
    -> JSONParser::animations::AnimationElementsFields {

    if (!animation_file.contains("animations") || animation_file["animations"].size() < num_animation ||
    ...
    return {.path = path, .name = name, .frame_number = frame_number, .frame_to_load = frame_to_load};
}

...
void Player::LoadAnimation(const std::string& filepath)
{
    ...
    for (int i = 0; i < number_of_animations; i++)
    {
        ...
        JSONParser::animations::AnimationElementsFields animation_characteristics =
        ...
        animations.at(animation_name)
            ->SetAnimationFrameNUmber(animation_characteristics.frame_to_load);

        for (int j = 0; j < animation_characteristics.frame_to_load; j++)
        {
            std::string texture_name =
                animation_characteristics.path + "frame" + std::to_string(j) + ".png";
...
void Player::Draw(SDL_Renderer* renderer)
{
    ...
    SDL_RenderTexture(renderer, current_animation->GetCurrentFrameTexture()->GetTexture(), nullptr,
                      &player_rect);
}

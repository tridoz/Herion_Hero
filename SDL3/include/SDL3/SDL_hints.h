 ...
 * backgrounded when the Apple TV remote's menus button is pressed, and when
 ...
 *   from the macOS menus bar, sending a SIGINT (ctrl-c) on Unix, etc.
...
/**
 * A variable that specifies the menus visibility when a window is fullscreen
 * in Spaces on macOS.
 *
 * The variable can be set to the following values:
 *
 * - "0": The menus will be hidden when the window is in a fullscreen space,
 *   and not accessible by moving the mouse to the top of the screen.
 * - "1": The menus will be accessible when the window is in a fullscreen
 *   space.
 * - "auto": The menus will be hidden if fullscreen mode was toggled on
 ...
 * menus). However, in case an invalid mnemonic is pressed, Windows makes an
 * audible beep to convey that nothing happened. This is true even if the
 * window has no menus at all!
 *
 ...
 * menus is opened from the keyboard, you will not receive a KEYUP event for
